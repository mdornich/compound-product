...
# Step 1: Find input source
log "Step 1: Finding input source..."
...
if [ -z "$ANALYZE_COMMAND" ]; then
  # Default mode: requires a report file
  LATEST_REPORT=$(ls -t "$REPORTS_DIR"/*.md 2>/dev/null | head -1)
  [ -f "$LATEST_REPORT" ] || error "No reports found in $REPORTS_DIR. Required when analyzeCommand is not set."
  REPORT_NAME=$(basename "$LATEST_REPORT")
  log "Using report: $REPORT_NAME"
else
  log "Using custom analyzeCommand (no report file required)"
  # Still check for optional report to pass as context
  LATEST_REPORT=$(ls -t "$REPORTS_DIR"/*.md 2>/dev/null | head -1) || true
  [ -f "$LATEST_REPORT" ] 2>/dev/null && REPORT_NAME=$(basename "$LATEST_REPORT")
fi
...
if [ -n "$ANALYZE_COMMAND" ]; then
  ...
  # Use /bin/bash to invoke scripts as data input, bypassing macOS Gatekeeper
  # quarantine (com.apple.provenance) that blocks launchd-spawned execution
  ANALYSIS_JSON=$(/bin/bash -c "/bin/bash $ANALYZE_COMMAND \"$LATEST_REPORT\"" 2>/dev/null)
else
  # Use default analyze script
  ANALYSIS_JSON=$(/bin/bash "$SCRIPT_DIR/analyze-report.sh" "$LATEST_REPORT" 2>/dev/null)
fi
...
log "Step 3: Creating feature branch from $BASE_BRANCH..."
...
/bin/bash "$SCRIPT_DIR/loop.sh" "$MAX_ITERATIONS" 2>&1 | tee "$OUTPUT_DIR/auto-compound-execution.log"
...
if [ -n "$REPORT_NAME" ]; then
  SOURCE_LINE="**Generated from report:** $REPORT_NAME"
else
  SOURCE_LINE="**Generated from:** custom analysis (analyzeCommand)"
fi
...
---
*This PR was automatically generated by Compound Product.*"
